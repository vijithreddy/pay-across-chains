...
import { startRace, startDryRace, type ChainRaceState } from "@/lib/race-engine";
...
const CONFETTI_CONFIG = { particleCount: 150, spread: 80, origin: { y: 0.4 }, colors: ["#7C3AED", "#A78BFA"] };
const DRY_RUN_RECIPIENT = "0xDEAD000000000000000000000000000000000000" as `0x${string}`;
...
export function RaceForm({
  ...
  const handleUpdate = useCallback(
    (chainId: number, update: Partial<ChainRaceState>) => {
      setChainStates((prev) => ({ ...prev, [chainId]: { ...prev[chainId], ...update } }));
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (dryRun) {
        await startDryRace({ recipient: (recipient || DRY_RUN_RECIPIENT) as `0x${string}`, amount, memo, onUpdate: handleUpdate });
      } else {
        await startRace({ recipient: recipient as `0x${string}`, amount, memo, account: address!, tempoClient, onUpdate: handleUpdate });
      }
    } catch (err) {
  ...
  const allDone = CHAIN_IDS.every((id) => chainStates[id]?.state === "confirmed" || chainStates[id]?.state === "error");
  ...
  return (
    <div className="space-y-4">
      {phase === "idle" && onBack && (
        <button onClick={onBack} className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-wider text-[var(--text-dim)] hover:text-[var(--text-secondary)] transition-colors">
          <ArrowLeft className="size-3" /> Back to checklist
        </button>
      )}

      {phase === "idle" && <PaymentForm recipient={recipient} setRecipient={setRecipient} amount={amount} memo={memo} allFunded={allFunded} racing={racing} onStart={handleStart} tempoAddress={tempoAddress} />}
      ...
      {allDone && (
        <button
          onClick={() => { setChainStates(makeInitialStates()); setPhase("idle"); confettiFired.current = false; }}
...
function PaymentForm({
  recipient, setRecipient, amount, memo, allFunded, racing, onStart, tempoAddress,
}: {
  recipient: string; setRecipient: (v: string) => void;
  amount: string; memo: string;
  allFunded: boolean; racing: boolean;
  onStart: () => void; tempoAddress?: `0x${string}`;
}) {
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-[1fr_120px_1fr] gap-2">
        <div>
          <label className="block text-[10px] font-mono uppercase tracking-wider text-[var(--text-dim)] mb-1">Recipient</label>
        ...
        <div>
          <label className="block text-[10px] font-mono uppercase tracking-wider text-[var(--text-dim)] mb-1">Amount</label>
          <div className="rounded-sm border border-[var(--border)] bg-[var(--bg-raised)] px-3 py-2 text-sm text-[var(--text-secondary)] font-mono">{amount} USDC</div>
        </div>
        <div>
          <label className="block text-[10px] font-mono uppercase tracking-wider text-[var(--text-dim)] mb-1">
            Memo <span className="ml-1.5 px-1.5 py-0.5 rounded-sm text-[8px] bg-[var(--tempo-dim)] text-[var(--tempo-bright)] border border-[var(--tempo-primary)]/30">TEMPO EXCLUSIVE</span>
          </label>
          <div className="rounded-sm border border-[var(--border)] bg-[var(--bg-raised)] px-3 py-2 text-sm text-[var(--text-secondary)] font-mono truncate">{memo}</div>
        </div>
      </div>
      <button onClick={onStart} disabled={!allFunded || !recipient || racing}
      ...
      {!allFunded && (
        <p className="text-[10px] font-mono text-[var(--text-dim)] text-center uppercase tracking-wider">
          {!tempoAddress ? "Connect Tempo Wallet to unlock" : "Fund all chains to unlock"}
        </p>
      )}
    </div>
  );
